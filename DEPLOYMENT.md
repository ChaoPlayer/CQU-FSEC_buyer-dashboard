...
### 5. 测试工时管理功能
工时管理模块是系统新增的核心功能，为用户和管理员提供工作时间的记录、提交、审批与统计。

#### 用户端功能
1. **访问工时管理页面**：登录后，点击导航栏“工时管理”进入 `/hours` 页面。
2. **查看工时统计**：
   - **考勤工时**：预留展示区域，未来由管理员统一分发。
   - **工作工时**：展示用户通过审批获得的工作工时总和。
   - **总工时**：考勤工时与工作工时之和。
3. **提交工作申请**：
   - 填写工作标题、工作描述（可选）。
   - 上传工作相关文件（支持任意格式，最大 10MB）。
   - 点击“提交”后，申请状态为“待审核”。
4. **查看历史申请**：页面下方列出最近提交的工作申请及其状态（待审核/已批准/已拒绝）。

#### 管理员端功能
1. **进入管理员面板**：使用管理员账号登录后，点击“管理员面板”。
2. **切换至工时管理选项卡**：在管理员面板顶部，选择“工时管理”选项卡。
3. **查看统计图表**：
   - **各组总工时排名**：横向柱状图展示各部门/组的总工时（按用户所在组汇总）。
   - **队员工时排名**：表格列出每位员工的工时总数，按降序排列。
   - **线上工作工时兑换申请数**：显示当前待处理的申请数量。
   - **工时预警**：预留占位区域，未来可接入预警规则。
4. **审批工作申请**：
   - 切换至“审批”子选项卡。
   - 使用分组筛选器按组过滤申请。
   - 勾选多个申请进行批量操作（批准、拒绝、下载文件）。
   - 点击单条申请的操作按钮（批准、拒绝、查看详情）。
   - 批准时需输入兑换工时数（单位：小时，支持小数），系统将自动创建工时记录并通知用户。
   - 拒绝时可填写拒绝理由（预留字段，当前未实现）。
5. **下载文件**：点击“下载文件”可下载用户上传的工作文件。

#### 通知功能
- **新工作申请通知**：当用户提交工作申请后，所有管理员会收到一条站内通知。
- **工时兑换通知**：当管理员批准申请并兑换工时后，申请人会收到一条站内通知，告知已获得相应工时。

#### 数据库与API
- **新增数据表**：`WorkSubmission`（工作申请）和 `HourRecord`（工时记录）。
- **新增API路由**：
  - `GET /api/work-submissions` – 获取工作申请列表
  - `POST /api/work-submissions` – 提交新申请
  - `PATCH /api/work-submissions/[id]` – 审批申请
  - `GET /api/hour-records` – 获取工时统计（支持按组、按用户聚合）
- **图表库**：使用 `recharts` 绘制统计图表，已自动安装。

#### 常见问题
- **图表不显示**：确保已安装 `recharts`（`npm install recharts`）。
- **无法提交文件**：检查 `/public/uploads` 目录权限，确保上传 API 正常工作。
- **通知未收到**：确认用户角色是否为管理员，或检查 `Notification` 表中是否生成了记录。
